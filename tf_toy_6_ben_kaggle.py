...
def import_and_save_dataset():
    ...
    """
    # Import dataset
    input_dataframe_test = pd.read_csv('input_test_set.csv', sep=',')
    # output_dataframe = pd.read_csv('output_training_set.csv', sep=',')

    x_pairs_test, y_pairs_test = make_pair_of_players(input_dataframe_test, None)

    # Save to a csv file
    x_pairs_test.to_csv('save_x_pairs_test.csv', header=True, index=True, mode='w')
    y_pairs_test.to_csv('save_y_pairs_test.csv', header=True, index=True, mode='w')
...
if __name__ == '__main__':
    ...
    x_test = pd.read_csv('save_x_pairs_test.csv', sep=',', index_col=0)
    ...
    x_test["same_team"] = (x_test["same_team"] - 0.5)*2
    # x["max_cs"] = (x["max_cs"] + 1) / 2
    x_test = x_test.drop(columns=["dist_tm_avg", "dist_tm_min", "dist_opp_min_sender"])
    ...
    model.train(x_train, y_train, None, None, sample_weights_train, None)

    pred = model.model.predict(x_test)

    #print(y_test.shape[0])

    probas = pred.reshape(int(x_test.shape[0]/22), 22)

    pred_players = np.argmax(probas, axis=1) + 1

    # one_hot_y = y_test.reshape(int(x_test.shape[0]/22), 22)
