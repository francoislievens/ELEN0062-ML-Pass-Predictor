...
def pass_distance(pairs, dist_mat):
    ...
    # Get sender and rec index in pairs:
    senders = pairs['sender'].iloc[:]
    recs = pairs['player_j'].iloc[:]
    # Indexing from zero
    senders -= 1
    recs -= 1
    start = 0
    ...
    for i in range(0, dist_mat.shape[0]):
        np_pairs[start+i*22:end+i*22, idx] = dist_mat[i][int(senders[i])]
...
def dist_tool(pairs, dist):
    ...
    for i in range(0, pairs.shape[0]):
        sender = int(pairs['sender'].iloc[i] - 1)
        rec = int(pairs['player_j'].iloc[i] - 1)
        pass_idx = int(pairs['pass_index'].iloc[i])
        # Get distances
...
def gravity_center(pairs, orig_data):
    ...
    for i in range(0, np_pairs.shape[0]):
        pass_id = int(pairs['pass_index'].iloc[i])
        sender = int(pairs['sender'].iloc[i] - 1)
        if sender < 12:
...
def is_between(pairs, orig_data):
    ...
    for i in range(0, pairs.shape[0]):
        sender = int(pairs['sender'].iloc[i] - 1)
        sender_x = int(pairs['x_sender'].iloc[i])
        sender_y = int(pairs['y_sender'].iloc[i])
        rec = int(pairs['player_j'].iloc[i] - 1)
        rec_x = int(pairs['x_j'].iloc[i])
        rec_y = int(pairs['y_j'].iloc[i])
        pass_id = int(pairs['pass_index'].iloc[i])
        sub_data_team = []
        sub_data_opp = []
        for j in range(0, 22):
